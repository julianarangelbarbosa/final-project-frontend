...
function EditBankStock() {
    ...
    const getBankStock = async () => {
        try {
            const response = await axios.get(`${process.env.REACT_APP_API_URL}/bank/list/${id}`);
            setComments(response.data.title);
      ...
      const handleSubmit = async (e)  => {
        e.preventDefault();
        try {
            await axios.put(`${process.env.REACT_APP_API_URL}/bank/list/${id}`,{comments});
            setComments('');
